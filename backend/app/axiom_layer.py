...
def _resolve(obs: dict) -> list[dict]:
    ...
    for wid, fa in _anomaly_frames(obs["timeseries"]).items():
        w = wh.get(wid, {})
        for n in obs["news"]:
            dist = abs(n["frame"] - fa)
            if dist > 4:
                continue
            b = canon(n["body"])
            s = 3 if (w.get("id", "") in n["body"] or canon(w.get("name", "")) in b) else 0
            s += 2 if canon(w.get("region", "")) in b else 0
    ...
    for _dist, _negs, wid, nid, fa in scored:
        ...
        hits = sorted(s["id"] for s in delayed
                      if s["warehouse_id"] == wid and abs(s["dispatch_frame"] - fa) <= _DELAY_TOL)
        if hits:
...
def naive_context(obs: dict) -> str:
    """Raw multi-store facts — present but unresolved; the model must canonicalize + join itself."""
    lines = ["[新闻]"]
    for n in obs["news"]:
        lines.append(f"- {n['id']}(第{n['frame']}帧):{n['body']}")
    lines.append("[运单表 · 仅延误]")
    for s in obs["sql"]["shipments"]:
        if s.get("status") == "delayed":
            lines.append(f"- {s['id']} 仓库={s['warehouse_id']} 发运帧={s['dispatch_frame']} 状态=delayed")
    lines.append("[仓库表]")
    for w in obs["sql"]["warehouses"]:
        lines.append(f"- {w['id']} 名称={w['name']} 区域={w['region']} 港口={w['port']}")
    lines.append("[吞吐量异常(原始信号)]")
    for wid, fr in _anomaly_frames(obs["timeseries"]).items():
        lines.append(f"- {wid} 在第{fr}帧吞吐量骤降")
    return "\n".join(lines)


def axiom_context(obs: dict) -> str:
    """Canonical-resolved + pre-joined facts — compact; the model just reads the answer off."""
    facts = _resolve(obs)
    lines = ["[已解析跨源事实 · 新闻→仓库→异常→延误运单]"]
    for f in facts:
        w = next((w for w in obs["sql"]["warehouses"] if w["id"] == f["warehouse_id"]), {})
        lines.append(
