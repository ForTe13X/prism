...
# lives in the spec's ``vocab`` + ``roles`` blocks, so a new domain = a new data_source spec (zero
# generator code). ``roles`` map the pattern's slots to a domain's store/field names so the downstream
# discriminability/solvers stay generic. Defaults below reproduce the logistics scenario verbatim.
_DEFAULT_VOCAB = {
