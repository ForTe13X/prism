...
- **未做(诚实):** §3 PDF/NoSQL 模态、§7 第 2/3 场景(需把生成器**泛化**为 scenario-generic,触动核心 hub,留作 serial 后续)、RESEARCH 的**完整研究**(跨模型矩阵 + 多 seed CI + $ 定价 cost-frontier + **学习式** axiom 层做真 build 摊销)。当前 axiom 层为算法式(build≈0)。
