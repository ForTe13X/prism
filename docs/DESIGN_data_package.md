...
- **未做(诚实):** §4b 真实校准(分布手设)、§5 agentic 解析器、§3 PDF/NoSQL 模态、§7 第 2/3 场景、RESEARCH 的 **LLM-ablation**(naive-RAG vs axiom-RAG)。`linked` 现为基础确定性联结器,**正是未来 axiom 层该改进、benchmark 该测出增益的位置**。
