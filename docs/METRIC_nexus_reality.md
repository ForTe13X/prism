...
`backend/app/nexus_xdom_view.py` + `GET /api/nexus_xdom/fdr_check` + 测试。OBSERVER §13 跑 N-域多重比较照出一条缝:旧 `_fires` 是**逐包 top-10% 分位**点火——**任何一对(哪怕零真耦合)都被强行点亮约 8 个「高」桥**(实测真对 8.27 vs 零对 8.27,**数量根本不分真假**;只有排序分)。外推 N 域:假高 ∝ C(N,2) 而真桥稀疏 ⇒ precision 崩(N=8 屏上 ~231 假 nexus vs ~5 真)。**这正是 METRIC §2 五透镜里 CACE(置换/FDR/族控)该补、却没接上的缺口。**
...
**实测(30 seed,`fdr_check`)**:
| 点火规则 | 真对 平均「高」 | 真对 precision | 零对 平均「高」 |
|---|---|---|---|
| 旧 · 相对 top-decile | 8.27 | 0.657 | **8.27(不熄)** |
| 新 · 绝对+FDR | 4.27 | **0.961** | **0.033(熄灭)** |

- **决定性**:零耦合对从「~8 个自信假 nexus」降到 **0.033 ≈ 熄灭**——「只有已验证的桥发光」终于**为真**(而非「相对最亮」);同时真对 precision 0.66→**0.96**。代价是 recall 降(FDR 保守:4.27/10 真桥,排名高但未过 FDR 的进 dim 的 `medium`,不发光)——**诚实的 precision↔recall 取舍,如实报**。
