...
### 11d. H2 能力轴:**质量增益随能力缩小,token 省是结构性的**(预注册;前沿点待跑)
`run_protocol().h2_capability_vs_gain` + [`docs/PREREG_axiom_gain_frontier.md`](PREREG_axiom_gain_frontier.md) + 测试。**唯一未测的泛化**(OBSERVER §15):活下来那条线的证据**几乎全是小本地模型**;**前沿模型本就能读乱上下文,还受益吗?** 先**预注册**(§6c 纪律:让「缩小」是被预测的结果,不是输),再跑前沿点。能力代理 = **每模型 naive-RAG F1**(无 axiom 层时的任务胜任度;params 在此是坏代理——qwen3-8b naive 已超 gemma-12b)。
...
- **H2a(质量增益随能力缩小)**:ΔF1 **单调下降**(0.166→0.157→0.108,Spearman −1.0)——更强的模型自己就能从原始上下文抠出更多答案,留给 resolver 的空间更小。**脆弱性诚实**:第一步 12b→qwen 仅差 **0.009(落在格内 CI 里)**——单调旗主要靠 **31b 那一跌**撑;「Spearman −1.0」读作 3 点排序、非有功效应。能力代理是 **task-local**(测自同一 fixtures,指「本任务胜任度」非泛能力)。
- **H2b(token 省是结构性的 ⇒ 与模型无关)**:token 省由**上下文大小**定(axiom ≈ naive 的 40%),**~持平**(spread 0.019 < 0.05)。**关键诚实**:头条「省 61%」是**结构性的、任何模型都成立**;H2a 测的是**质量**那半还在不在。
- **诚实状态**:上表是**已观测、非预注册**(fixtures 早于本假设),且只跨 8B–31B 本地模型 ⇒ **趋势支持 H2,但非预注册确认**。**预注册的测试 = 一个更高能力点**(前沿 API 或最大可安全加载的本地模型)。因本机曾在试探前沿本地模型时**死机**,前沿点**待跑**(需安全端点);跑了无论增益有无都**保留并报**(无增益=被证实的 H2a,不静默删格)。
- **赢面**:成立=真跨能力结果(结构 61% 存活、质量增益正确地集中在更弱模型上);反转=更值钱的意外(强模型也受益)。两者都报。
